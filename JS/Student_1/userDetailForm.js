document.getElementsByClassName('btn-purchase')[0].addEventListener('click',checkMyCart)
                ...
                function runModel(){
                    openModelButtons.forEach(button=>{
                        const model = document.querySelector(button.dataset.modelTarget)    //assigning the created model to a variable using queryselector and data target attribute 
                ...
                closeModelButtons.forEach(button=>{
                        button.addEventListener('click', () => {
                            const model = button.closest(".model")       //checks the closest parent with the class model and assigns it to a variable
